...
mortality<- flare %>% group_by(treatment, site, region) %>%
  ...
  left_join(select(flare, site, region )) %>%  distinct() %>% 
  mutate(mortality_mean = round(100*(unburned-burned)/unburned,1),
         mortality_SE = NA ) %>% select(site, region, mortality_mean, mortality_SE)%>% group_by(region) %>% 
  group_modify(~ .x %>% add_row(site = 'regional mean', 
                                mortality_mean =
                                  round(mean(.x$mortality_mean , na.rm = TRUE),2),
                                mortality_SE = round(sd(.x$mortality_mean, na.rm=TRUE )/
                                                       sqrt(sum(!is.na(.x$mortality_mean))),2)))%>% 
  ungroup() %>%
  mutate(mortality = ifelse(is.na(mortality_SE),
                            sprintf("%.1f",mortality_mean),
                            paste(sprintf("%.1f",mortality_mean),
                                  sprintf("%.1f",mortality_SE), sep=" ± " ))) %>% 
  select(site, region, mortality) 
...
## fire severity and combustion
severity_dfs<- list(dnbr, crown_severity,mortality, AboveChanges, prop_AG_combusted, BelowChanges, prop_BG_combusted, totalmean_df, prop_combusted,
                    BG_combust_fract)
